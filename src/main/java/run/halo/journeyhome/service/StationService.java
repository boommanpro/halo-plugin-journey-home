...
public class StationService {
    ...
    public Mono<StationVo> create(JourneyStation station) {
        boolean nameAutoGenerated =
            station.getMetadata().getName() == null || station.getMetadata().getName().isBlank();
        ...
        // 自动生成的 slug 可能与其他站点冲突（如重名），自动追加序号保证唯一；
        // 用户显式指定的 name 冲突时仍按原样上抛 already-exists 错误
        Mono<JourneyStation> resolved = nameAutoGenerated
            ? resolveUniqueName(station.getMetadata().getName(), 0)
                .doOnNext(unique -> station.getMetadata().setName(unique))
                .thenReturn(station)
            : Mono.just(station);
        return resolved.flatMap(client::create).map(StationService::toVo);
    }

    /** 若 name 已被占用，依次尝试 name-2、name-3 ... 直到找到空闲名称。 */
    private Mono<String> resolveUniqueName(String base, int attempt) {
        String candidate = attempt == 0 ? base : base + "-" + (attempt + 1);
        if (attempt > 98) {
            return Mono.just(base + "-" + System.currentTimeMillis());
        }
        return client.fetch(JourneyStation.class, candidate)
            .flatMap(exists -> resolveUniqueName(base, attempt + 1))
    ...
    public Mono<List<StationVo>> listAll() {
        return client.listAll(JourneyStation.class, new ListOptions(), Sort.unsorted())
            .map(StationService::toVo)
            .collectList()
    ...
    public Mono<PageConfigVo> savePageConfig(JourneyPageConfig.Spec spec) {
        if (spec == null) {
            return Mono.error(new ServerWebInputException("spec is required"));
        }
        return client.fetch(JourneyPageConfig.class, JourneyPageConfig.SINGLETON_NAME)
            .flatMap(existing -> {
